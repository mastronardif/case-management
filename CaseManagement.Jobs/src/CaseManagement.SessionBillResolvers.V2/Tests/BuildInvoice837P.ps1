...
# Patient is a business entity, not a document (SourceDocumentIdREMOVEME/JsonDocumentIdREMOVEME
# on cases.Patient) — queried directly by CaseId, same as InsuranceCoverage/Provider. Feeds
# 2010CA only; 2010BA subscriber identity comes from InsuranceCoverage, not this table.
...
if ($sources.sessions -and $sources.sessions.Count -gt 0) {
    Write-Host ""
    Write-Host "Rendering Provider : loading for session $($sources.sessions[0])..." -ForegroundColor Yellow
    $renderingProvider = Get-RenderingProvider -SessionJsonDocId $sources.sessions[0]
    if ($renderingProvider) {
        ...
        Write-Host "  No clinicianUsername on session $($sources.sessions[0]), or no matching active Provider row." -ForegroundColor DarkYellow
    }
}
